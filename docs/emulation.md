...
All this is possible to patch, BUT, we cannot change header properties (UA etc) for navigation. And the best we can do is get our script to be the FIRST executed in the context.

But if we use Service Workers, we can basically do this with service workers. But there will be some CORS issues most likely.
