...
Possibly using WebRTC we can do video and shot. 
Logs and errors are eminently possible (using top level onerror handler).
Events are possible by overwriting the dialog functions, getting notes from our SW of failed requests and overwriting the window open function.
